#!/usr/bin/env python3
"""
Working Streaming TTS Server for CallWaiting.ai
Uses fallback TTS manager with system TTS for immediate functionality
"""
...
# Import our Fallback TTS Manager
sys.path.append(str(Path(__file__).parent))
from fallback_tts_manager import fallback_tts_manager, VoiceProfile
...
app = FastAPI(
    title="CallWaiting.ai Working Streaming TTS Service",
    description="Streaming TTS with voice management using system TTS fallback",
    version="2.0.0"
)
...
async def verify_api_key(authorization: str = Header(None)) -> str:
    ...
    for tenant_id, tenant_info in fallback_tts_manager.tenants.items():
...
async def startup_event():
    ...
    logger.info("🚀 Starting CallWaiting.ai Working Streaming TTS Service")
    model_info = fallback_tts_manager.get_model_info()
    logger.info(f"📊 TTS System: {model_info['system_tts']}")
    logger.info(f"👥 Tenants configured: {len(fallback_tts_manager.tenants)}")

@app.on_event("shutdown")
async def shutdown_event():
    """Cleanup on shutdown"""
    logger.info("🛑 Shutting down TTS service")
    fallback_tts_manager.cleanup()
...
async def root():
    ...
    model_info = fallback_tts_manager.get_model_info()
    return {
        "service": "CallWaiting.ai Working Streaming TTS Service",
        "version": "2.0.0",
        "status": "running",
...
async def list_voices(tenant_id: str = Depends(verify_api_key)):
    ...
    profiles = fallback_tts_manager.list_voice_profiles(tenant_id)
    return profiles
...
async def upload_voice(
    ...
    try:
        ...
        profile = fallback_tts_manager.create_voice_profile(
            tenant_id=tenant_id,
            name=name,
...
async def synthesize_text(
    ...
    try:
        # Get voice profile
        voice_profile = fallback_tts_manager.get_voice_model(tenant_id, request.voice_id)
        ...
        audio_chunks = []
        async for chunk in fallback_tts_manager.inference_stream(
            text=request.text,
    ...
    try:
        ...
        async def audio_streamer():
            try:
                ...
                # Send error as audio metadata (could be improved)
...
async def get_tenant_stats(tenant_id: str = Depends(verify_api_key)):
    ...
    tenant_info = fallback_tts_manager.get_tenant_info(tenant_id)
    if not tenant_info:
        raise HTTPException(status_code=404, detail="Tenant not found")
    
    voice_count = len(fallback_tts_manager.voice_profiles.get(tenant_id, {}))
...
async def generate_demo_audio(tenant_id: str = Depends(verify_api_key)):
    ...
    demo_texts = [
        {
            ...
            "filename": f"callwaiting_intro_{tenant_id}.wav",
        ...
        },
        {
            ...
            "filename": f"callwaiting_welcome_{tenant_id}.wav", 
            "description": "Welcome Message"
        },
        {
            "text": "Thank you for calling CallWaiting.ai. Your call is important to us. Please hold while we connect you.",
            "filename": f"callwaiting_hold_{tenant_id}.wav",
            "description": "Hold Message"
        },
        {
            "text": "This is a demonstration of our streaming TTS technology. The audio is generated in real-time using advanced voice synthesis.",
            "filename": f"callwaiting_streaming_demo_{tenant_id}.wav",
            "description": "Streaming Technology Demo"
        }
    ]
    ...
    try:
        ...
        voice_profile = fallback_tts_manager.get_voice_model(tenant_id, "default")
        if not voice_profile:
...
if __name__ == "__main__":
    print("🚀 Starting CallWaiting.ai Working Streaming TTS Service")
